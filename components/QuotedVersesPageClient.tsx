...
interface QuotedVersesPageClientProps {
  initialVerses: QuotedVerse[];
}

function QuotedVersesPageContent({ initialVerses }: QuotedVersesPageClientProps) {
  ...
  useEffect(() => {
    try {
      ...
      console.error('Error initializing quoted verses:', error);
      setLoadError('Хатоги дар боргирии маълумот');
    } finally {
      setIsLoading(false);
    }
  }, [initialVerses]);
  ...
  const scrollToVerse = (index: number) => {
    const scrollContainer =
      activeTab === 'simple' ? simpleScrollRef.current : styledScrollRef.current;
    if (scrollContainer) {
  ...
  const handlePlayVerse = async (verse: QuotedVerse) => {
    ...
    const isPlayingThisVerse =
      playbackState?.currentSurahNumber === surahNum &&
        ...
        {(['simple', 'styled'] as Tab[]).map((tab) => (
          <button
            ...
            style={{
              ...
              position: 'relative',
              color:
                activeTab === tab ? 'var(--color-primary)' : 'var(--color-text-primary)',
  ...
  return (
    <div
      style={{
        ...
        border: `1.5px solid ${
          isSelected ? 'var(--color-primary)' : 'var(--color-outline)'
        }`,
      ...
      >
        ...
        <div
          style={{
            ...
            bottom: 0,
            background:
              'linear-gradient(to top, rgba(0, 0, 0, 0.45) 0%, rgba(0, 0, 0, 0.25) 100%)',
          }}
        />
...
export default function QuotedVersesPageClient({
  initialVerses,
}: QuotedVersesPageClientProps) {
  return (
    <Suspense
      fallback={
        <div
    ...
    >
      <QuotedVersesPageContent initialVerses={initialVerses} />
    </Suspense>
  );
}

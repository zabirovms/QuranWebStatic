...
/**
 * Loads the detailed 99 Names of Allah JSON and caches intro/names list
 */
...
export async function getAllAsmaulHusna(): Promise<AsmaulHusna[]> {
  ...
  try {
    ...
    console.error('Error loading Asmaul Husna detailed names:', error);
    return [];
  }
}
...
export async function getAsmaulHusnaIntro(): Promise<AsmaulHusnaIntro | null> {
  ...
  try {
    ...
    console.error('Error loading Asmaul Husna intro headers:', error);
    return null;
  }
}

/**
 * Get a specific Name of Allah by its Latin slug.
 */
export async function getAsmaulHusnaBySlug(slug: string): Promise<AsmaulHusna | null> {
